...
<fieldset>
		 	...
		 	<fieldset>
		 	<legend><font style="color: orange"><%=sm.getMallDes() %>商品</font></legend>
...
<table border="0" cellpadding="3" cellspacing="1" width="100%" align="center" style="background-color: #b9d8f3;">
	...
	<tr bgcolor='#F4FAFF' style="text-align: center;">
		...
		<td><a href="#" onclick="removeGoods(<%=goods.getId()%>,<%=sm.getId() %>)">移除商品</a></td>
	</tr>
	<%} }else{%>
		<tr bgcolor='#F4FAFF' style="text-align: center;">
...
<script type="text/javascript">
	...
	function goPage(res){
		...
		var smId = <%=sm.getId()%>;
		document.getElementById("myForm").action="findSMGoods.do?curPage="+res+"&countPage="+countPage+"&smid="+smId;
		document.getElementById("myForm").submit();
	}

	function removeGoods(goodsId,mallId){
		
		Dialog.confirm("提示：确定从Banner中移除该商品吗？",function(){
			$.ajax({
				url:'removeMallGoods.do',
				data:{goodsId:goodsId,mallId:mallId},
				dataType:'text',
