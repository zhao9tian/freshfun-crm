...
window.onload = function(){
	$.ajax({
		type:"post",
		url:"http://www.freshfun365.com/freshfun-crm/withdraw/queryWithdrawList.do",
		dataType:'JSON',
			...
			$.each(stateOneArr, function(n,obj) {
				if($(stateOneArr[n]).html() == '变更为已处理'){
					$(stateOneArr[n]).click(function(){
						...
						$.ajax({
							type:"get",
							url:"http://www.freshfun365.com/freshfun-crm/withdraw/toHandled.do?operation=10&id="+id,
							dataType:'JSON',
							success:function(data){
								console.log(data);
				...
				if($(stateTwoArr[n]).html() == '驳回提现'){
					$(stateTwoArr[n]).click(function(){
						...
						$('.btn-Y').click(function(){
							...
							$.ajax({
								type:"get",
								url:"http://www.freshfun365.com/freshfun-crm/withdraw/toReject.do?operation=20&id="+id+"&rejectReason="+rejectReason,
								dataType:'JSON',
								success:function(data){
									console.log(data);
//									window.location.reload();
